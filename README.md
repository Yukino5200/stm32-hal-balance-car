...
> **关于本项目的来历**
>
> 本项目以 B 站 UP 主**江协科技（江科大）**的 STM32 标准库平衡车项目为原型，教学视频见 https://www.bilibili.com/video/BV1kBanzfExg/ 。该版基于**标准库**、手写工程；而这一版是我在此基础上做的**移植与重构**：改用 **HAL 库 + FreeRTOS**，工程用 **CubeMX 生成 + CLion 开发**。
>
...
完整原理图见 [`docs/平衡车控制板-V2.0.pdf`](docs/平衡车控制板-V2.0.pdf)（**该图纸版权归江协科技所有，此处仅供学习交流，请勿商用**）。下表的**原理图网名**一列就是图上标的那些标号（`AIN1`、`O_SCL` 之类），对着图找元件时用得上。
...
- `docs/` 下的原理图版权归**江协科技**所有，仅随本仓库收录供学习交流，请勿商用。
- 硬件清单、控制方案与代码逻辑的实现**均以江协科技官方为准**。本仓库所做的是把它移植到 HAL 库 + FreeRTOS，并调整工程结构，不涉及控制方案本身的原创。
- 若本仓库的描述与官方内容有出入，**一律以官方为准**。若权利人认为本仓库的收录方式不妥，请联系删除。
